...
export default function MasculinosPage({
  ...
  // Obter todos os produtos masculinos
  const allProducts = getAllProducts()
  let masculineProducts = allProducts.filter((product) => product.category === "Masculino")
  ...
  if (marcas.length > 0) {
    masculineProducts = masculineProducts.filter((product) => marcas.includes(product.brand))
  }
  ...
  if (familias.length > 0) {
    masculineProducts = masculineProducts.filter(
  ...
  masculineProducts = masculineProducts.filter((product) => {
  ...
  if (ordenar) {
    switch (ordenar) {
      ...
      case "preco_desc":
        ...
        break
      case "nome_asc":
        masculineProducts.sort((a, b) => a.name.localeCompare(b.name))
        break
      case "nome_desc":
        masculineProducts.sort((a, b) => b.name.localeCompare(a.name))
        break
      case "avaliacao":
        masculineProducts.sort((a, b) => b.rating - a.rating)
        break
      case "novidades":
        masculineProducts.sort((a, b) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0))
        break
      default:
        // Ordenação padrão: destaques
        masculineProducts.sort((a, b) => (b.isFeatured ? 1 : 0) - (a.isFeatured ? 1 : 0))
    }
  }
